...
// Href Link to open Tabs
$(document).ready(function() {
	// Function to activate tab based on tab name
	function activateTab(tabName) {
	  $('.titles .title').filter(function() {
		return $(this).text().trim() === tabName;
	  }).click();
	  
	}
  
	// Activate tab if "tab" parameter is present in the URL
...
Array.from(accordion).forEach(element => element.addEventListener('click', function() {	
	...
	} accPanel.classList.toggle('acc-transition')
}))
...
const bacp = document.querySelectorAll('#bacp');
